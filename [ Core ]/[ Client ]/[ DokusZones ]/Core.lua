...
function DokusZones.drawPoly(poly)
...
local function _pointInPoly(point, poly)
  ...
  if poly.grid then
    ...
    local gridCellX = (gridPosX * GD) // size.x
    local gridCellY = (gridPosY * GD) // size.y
...
local function _createGrid(poly, options)
  Citizen.CreateThread(function()
    ...
    if options.Grid then
      ...
      print("[DokusZones] Debug: Grid Coverage at " .. coverage .. "% with " .. poly.GD
...
local function _calculatePoly(poly, options)
  ...
  if poly.useGrid then
    ...
    poly.gridArea = 0.0
    poly.gridCellWidth = poly.size.x / poly.GD
    poly.gridCellHeight = poly.size.y / poly.GD
...
function _initDebug(poly, options)
  local PO, GR = false, false
  if (options.Poly == 1) then PO = true else PO = false end
  if (options.Grid == 1) then GR = true else GR = false end
  local debugEnabled = (PO or GR)
  ...
  Citizen.CreateThread(function()
    ...
    while not poly.destroyed do
      ...
      if not isEntityZone and GR and poly.lines then
        _drawGrid(poly)
      end
      Citizen.Wait(0)
    end
...
function DokusZones:Create(points, options)
  ...
  local poly = {
    Type = tostring(options.Type) or nil,
    City = tostring(options.City) or nil,
    Name = tostring(options.Name) or nil,
    OnEnter = options.OnEnter,
    OnExit = options.OnExit,
    Meta = options.Meta or false,
    points = points,
    center = vector2(0, 0),
    size = vector2(0, 0),
    max = vector2(0, 0),
    min = vector2(0, 0),
    minZ = tonumber(options.MinZ) or (Coords[3] - 1.0),
    maxZ = tonumber(options.MaxZ) or (Coords[3] + 2.0),
    useGrid = useGrid,
    GD = tonumber(options.GD) or 30,
    debugColors = {
      walls = colors.walls or {0, 255, 0},
      outline = colors.outline or {255, 0, 0},
      grid = colors.grid or {255, 255, 255}
    },
    Poly = options.Poly or 0,
    Grid = options.Grid or 0,
...
function DokusZones:CreateAroundEntity(entity, options)
  ...
  local poly = DokusZones:Create(points, options)
...
function DokusZones:isPointInside(point)
  if self.destroyed then
    print("[DokusZones] Warning: Called isPointInside on destroyed zone {name=" .. self.name .. "}")
    return false
  end
...
function DokusZones:destroy()
  self.destroyed = true
  if self.Poly or self.Grid then
    print("[DokusZones] Debug: Destroying zone {name=" .. self.name .. "}")
  end
end


-- Helper functions
function DokusZones.getPlayerPosition()
  return GetEntityCoords(PlayerPedId())
end

local HeadBone = 0x796e;
function DokusZones.getPlayerHeadPosition()
  return GetPedBoneCoords(PlayerPedId(), HeadBone);
end

function DokusZones:onPointInOut(getPointCb, onPointInOutCb, waitInMS)
...
function DokusZones:onEntityDamaged(onDamagedCb)
  local entity = self.entity
  if not entity then
    print("[DokusZones] Error: Called onEntityDamage on non entity zone {name=" .. self.name .. "}")
    return
  end
...
function DokusZones:getBoundingBoxMin()
  return self.min
end

function DokusZones:getBoundingBoxMax()
  return self.max
end

function DokusZones:getBoundingBoxSize()
  return self.size
end

function DokusZones:getBoundingBoxCenter()
  return self.center
end
