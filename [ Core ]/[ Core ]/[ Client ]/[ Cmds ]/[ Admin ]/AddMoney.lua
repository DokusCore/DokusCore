...
RegisterCommand("addmoney", function(source, args, rawCommand)
  ...
  local function DoThis()
    ...
    if not args[4] then ErrUsageMsg(_('Error_AddMoneyAmount', User.Language), _('Usage_AddMoney', User.Language)) return end
    ...
    elseif (string.lower(args[3]) == 'wallet') then Type = 'wallet' else
      ...
      TriggerServerEvent('DokusCore:S:Core:AddMoney', Steam[1], sID, cID, Type, math.floor(tonumber(args[4])))
    end
